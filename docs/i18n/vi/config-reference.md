...
Kênh iMessage qua máy chủ BlueBubbles tự lưu trữ.

| Khóa                 | Bắt buộc | Mục đích                                                                                            |
| -------------------- | -------- | --------------------------------------------------------------------------------------------------- |
| `server_url`         | Có       | URL máy chủ BlueBubbles (ví dụ `http://192.168.1.100:1234`)                                         |
| `password`           | Có       | Mật khẩu máy chủ BlueBubbles                                                                        |
| `allowed_senders`    | Tùy chọn | Allowlist người gửi — số điện thoại hoặc Apple ID (`[]` = cho phép tất cả khi `dm_policy = "open"`) |
| `webhook_secret`     | Tùy chọn | Token bí mật Bearer để xác thực webhook (`/bluebubbles`)                                            |
...
| `group_allow_from`   | Tùy chọn | Danh sách GUID chat nhóm khi `group_policy = "allowlist"`. Dùng `["*"]` cho tất cả                  |
| `send_read_receipts` | Tùy chọn | Gửi biên nhận đã đọc sau mỗi tin nhắn được xử lý. Mặc định: `true`                                  |
