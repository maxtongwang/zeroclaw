...
impl LeakDetector {
    ...
    fn check_api_keys(&self, content: &str, patterns: &mut Vec<String>, redacted: &mut String) {
        ...
        let regexes = API_KEY_PATTERNS.get_or_init(|| {
            vec![
                ...
                // Generic
                (Regex::new(r"api[_-]?key[=:][[:space:]]*[a-zA-Z0-9_-]{20,}").unwrap(), "Generic API key "),
            ]
        });
    ...
    fn check_aws_credentials(&self, content: &str, patterns: &mut Vec<String>, redacted: &mut String) {
        ...
        let regexes = AWS_PATTERNS.get_or_init(|| {
            vec![
                ...
                (Regex::new(r"aws[_-]?secret[_-]?access[_-]?key[=:][[:space:]]*[a-zA-Z0-9/+=]{40}").unwrap(), "AWS Secret Access Key "),
            ]
        });
    ...
    fn check_generic_secrets(&self, content: &str, patterns: &mut Vec<String>, redacted: &mut String) {
        ...
        let regexes = SECRET_PATTERNS.get_or_init(|| {
            vec![
                (Regex::new(r"(?i)password[=:][[:space:]]*[^[:space:]]{8,}").unwrap(), "Password in config "),
                (Regex::new(r"(?i)secret[=:][[:space:]]*[a-zA-Z0-9_-]{16,}").unwrap(), "Secret value "),
                (Regex::new(r"(?i)token[=:][[:space:]]*[a-zA-Z0-9_.-]{20,}").unwrap(), "Token value "),
            ]
        });
